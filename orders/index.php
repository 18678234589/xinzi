...
if (($_GET['ajax'] ?? '') === 'modules' && $ajax_employee_id > 0) {
    ...
    if ($modCfg && !empty($modCfg['modules'])) {
        foreach ($modCfg['modules'] as $m) {
            if (in_array($m['type'], ['standard','tiered','per_order','profit_commission','trademark_commission','trademark_cashback','referral_order','customer_reward','miniprogram_commission']) && ($m['enabled'] ?? true)) {
                $extra = '';
                    ...
                    <div class="form-group" id="uploadProjectGroup">
                        ...
                        <div id="uploadProject" class="project-checkbox-list">
                            ...
                            if ($locked_employee):
                                ...
                                if ($modCfg && !empty($modCfg['modules'])):
                                    foreach ($modCfg['modules'] as $m):
                                        if (in_array($m['type'], ['standard','tiered','per_order','profit_commission','trademark_commission','trademark_cashback','referral_order']) && ($m['enabled'] ?? true)):
                                            $modName = $m['name'];
                                            $extra = '';
                                            if ($m['type'] === 'standard' && isset($m['config']['rate']) && $m['config']['rate'] !== '') {
