        ...
        if ($action === 'preview') {
            ...
            foreach ($raw as $index => $row) {
                ...
                try {
                    ...
                    if ($record['order_no'] === '' || strlen($record['order_no']) > 100 || !$record['order_date'] || ($record['contract_amount'] !== '' && !preg_match('/^\d+(?:\.\d{1,2})?$/', $record['contract_amount'])) || (float)$record['contract_amount'] > 999999999999.99 || !in_array($status, ['已完成','未完成'], true)) throw new RuntimeException('订单号、日期、售价或状态无效');
                    if ($existing) {
                    ...
                    if (!$existing && $actor['role'] === 'customer_service' && ps_is_website_order($selectedBusiness)) {
                        if (!$record['people']['technical']) throw new RuntimeException('网站客服导入新订单须指定接单技术');
                    ...
                    $record['domain_mode'] = !$businessDefinition['resources'] ? 'none' : ($resourceSelection && !$record['resource_locked'] ? ps_import_domain_mode($record['domain_used']) : 'pending');
            ...
            foreach ($preview as $row) {
                ...
                $choice = $needsResources ? (string)($choices[$line] ?? ($row['domain_mode'] === 'none' ? 'none' : ($row['domain_template_id'] ?: ''))) : 'none';
                ...
                foreach ($ready as [$row, $domainTemplate, $serverTemplate]) {
                    ...
                    if ($existing) {
                        ...
                        if ($resourceSelection) {
                            ...
                            if ($mode === 'pending') {
                                ps_intake_confirm_resources($orderId, $domainTemplate ? 'template' : 'none', $domainTemplate['id'] ?? 0, $serverTemplate['id'] ?? 0, $actor);
                            } elseif ($mode === false) {
                                ps_intake_save_resources($orderId, 'excel', (int)$row['line'], $domainTemplate, $serverTemplate, is_numeric($row['ssl_used']) ? $row['ssl_used'] : null);
                    ...
                    $insertOrder->execute([$row['order_no'], $row['payment_nickname'], $row['project_type'], $row['shop'], $row['contract_amount'] === '' ? 0 : $row['contract_amount'], $row['order_date'], $row['delivery_status'], implode('；', $noteParts), $actor['role'] === 'finance' ? $actor['id'] : null]);
                    $orderId = (int)$pdo->lastInsertId();
                    ps_source_record($orderId, $row['contract_amount'] === '' ? 'missing' : 'manual', $row['payment_nickname'], '');
                    ...
                    if ($businessDefinition['resources']) ps_intake_save_resources($orderId, 'excel', (int)$row['line'], $domainTemplate, $serverTemplate, is_numeric($row['ssl_used']) ? $row['ssl_used'] : null, $actor['role'] === 'customer_service' ? 'pending' : null);
