...
<div class="form-row" data-for="fixed per_unit base_fee attendance_bonus"><div class="form-group col-md-3"><label id="monthlyAmountLabel">金额 ¥</label><input name="amount" type="number" step="0.01" min="0" class="form-control" value="<?php echo e($form['params']['amount'] ?? ''); ?>"></div><div class="form-group col-md-6 pt-4" data-for="fixed"><label class="mb-0"><input type="checkbox" name="separate" value="1" <?php echo !empty($form['params']['separate']) ? 'checked' : ''; ?>> 另行支付（如法人补助），单列展示、不计入应结算金额</label></div></div>
