...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'preview_file') {
            $file = $_FILES['file'] ?? [];
            if (($file['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK || (int)($file['size'] ?? 0) > 5 * 1024 * 1024) throw new RuntimeException('请选择不超过 5 MB 的 XLSX 或 CSV 退款表');
            ...
            $_SESSION['ps_refund_preview'] = $preview; $_SESSION['ps_refund_file'] = 0; $_SESSION['ps_refund_owner'] = $owner;
            ...
            unset($_SESSION['ps_refund_preview'], $_SESSION['ps_refund_file'], $_SESSION['ps_refund_owner']);
            $preview = []; $success = $isFinance ? '已审核登记 ' . $done . ' 笔支付宝退款；已审核订单的分成差额计入 ' . $month . '。' : '已提交 ' . $done . ' 笔支付宝退款，等待财务核对。审核前不会扣减订单或项目报酬。';
        } elseif ($action === 'review') {
            if (!$isFinance) throw new RuntimeException('仅财务可审核');
            ps_refund_review((int)($_POST['refund_id'] ?? 0), (string)($_POST['decision'] ?? ''), $actor, $month);
            $success = '退款审核已完成';
...
<div class="row"><div class="col-lg-6 mb-3"><div class="card project-form-card h-100"><div class="card-body"><div class="project-section-title"><span class="project-step">01</span><div><h5>上传已有退款表</h5><p>支持 XLSX / CSV，自动按订单号找原项目并核对可退余额；先预览，再勾选确认。</p></div></div>
...
<small class="text-muted d-block mt-2">表中如有“退款方式”列，只接收支付宝行；缺少该列时，本入口按支付宝线下退款核对。原文件留在“原始表格”。</small></div></div></div>
