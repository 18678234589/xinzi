...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'template') {
            ...
            try {
                foreach ($people as $person) {
                    ...
                    $password = $username; // 默认密码 = 登录名，首次登录绑定手机后可自行修改
        ...
        } elseif ($action === 'account_second_business') {
            $userId = (int)($_POST['user_id'] ?? 0);
            $secondBusiness = (string)($_POST['second_business'] ?? '');
            $q = db()->prepare('SELECT business_name FROM project_user_businesses WHERE user_id=? ORDER BY is_default DESC,business_name LIMIT 1');
            $q->execute([$userId]);
            $primary = (string)$q->fetchColumn();
            if ($primary === '' || ($secondBusiness !== '' && (!isset(ps_business_catalog()[$secondBusiness]) || $secondBusiness === $primary))) throw new RuntimeException('请选择与主要业务不同的有效第二业务');
            db()->beginTransaction();
            try {
                db()->prepare('DELETE FROM project_user_businesses WHERE user_id=? AND is_default=0')->execute([$userId]);
                if ($secondBusiness !== '') db()->prepare('INSERT INTO project_user_businesses (user_id,business_name,is_default) VALUES (?,?,0)')->execute([$userId, $secondBusiness]);
                ps_audit('account', $userId, 'second_business', $actor, ['primary' => $primary, 'second_business' => $secondBusiness]);
                db()->commit();
            } catch (Throwable $e) { db()->rollBack(); throw $e; }
            $success = $secondBusiness === '' ? '第二业务已移除' : '第二业务已关联；该人员现在可以上传两种业务的订单表';
        } elseif ($action === 'account_update') {
            $userId = (int)($_POST['user_id'] ?? 0);
            $role = (string)($_POST['role'] ?? '');
            $businessName = (string)($_POST['business_name'] ?? '');
            $secondQuery = db()->prepare('SELECT business_name FROM project_user_businesses WHERE user_id=? AND is_default=0 ORDER BY business_name LIMIT 1');
            $secondQuery->execute([$userId]);
            $secondBusiness = (string)($_POST['second_business'] ?? $secondQuery->fetchColumn());
            $active = isset($_POST['is_active']) ? 1 : 0;
            $password = (string)($_POST['password'] ?? '');
            if (isset($_POST['reset_default'])) {
                // 忘记密码时重置为默认密码 = 登录名（姓名拼音），本人登录后可再修改
                $nameQuery = db()->prepare('SELECT username FROM project_users WHERE id=?');
                $nameQuery->execute([$userId]);
                $password = (string)$nameQuery->fetchColumn();
                if ($password === '') throw new RuntimeException('账户不存在');
            } elseif ($password !== '' && strlen($password) < 10) throw new RuntimeException('新密码至少10位（或勾选“重置为默认密码”）');
            if (!in_array($role, ['technical','customer_service'], true) || !isset(ps_business_catalog()[$businessName]) || ($secondBusiness !== '' && (!isset(ps_business_catalog()[$secondBusiness]) || $secondBusiness === $businessName))) throw new RuntimeException('请选择不同的有效业务类型');
            db()->beginTransaction();
            try {
            if ($password === '') {
            ...
            if ($secondBusiness !== '') db()->prepare('INSERT INTO project_user_businesses (user_id,business_name,is_default) VALUES (?,?,0)')->execute([$userId, $secondBusiness]);
            ps_audit('account', $userId, 'update', $actor, ['role' => $role, 'business' => $businessName, 'second_business' => $secondBusiness, 'is_active' => $active, 'password_changed' => $password !== '']);
            db()->commit();
            } catch (Throwable $e) { db()->rollBack(); throw $e; }
            $success = '账户已更新';
...
$accounts = db()->query('SELECT u.id,u.username,u.phone,u.password_changed_at,u.role,u.is_active,e.name,e.department,(SELECT b.business_name FROM project_user_businesses b WHERE b.user_id=u.id ORDER BY b.is_default DESC,b.business_name LIMIT 1) business_name,(SELECT b.business_name FROM project_user_businesses b WHERE b.user_id=u.id AND b.is_default=0 ORDER BY b.business_name LIMIT 1) second_business FROM project_users u JOIN employees e ON e.id=u.employee_id ORDER BY e.department,e.name')->fetchAll();
