...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'set_pool_opening') {
            ...
            $activeRotation = pg_active_rotation();
            $quarter = $activeRotation['start_date'] ?? pg_quarter_start();
            if ($quarter >= '2026-09-15') throw new RuntimeException('本任期奖金目标按已确认规则自动建立，不能在这里改写期初额度');
...
<div class="governance-page governance-ideas-page">
  ...
  <section class="governance-card" id="pool"><div class="governance-heading"><div><span class="governance-step">本任期奖金池</span><h2><?php echo e($quarterStart); ?> — <?php echo e($termEnd); ?> · 董事长尽职奖金池</h2><p class="governance-hint">金额只属于本届轮值董事长，不跨任期分摊。每个完整六天轮值窗口没有有效提交，系统自动记 −¥300；监委会可逐笔豁免。</p></div><span class="governance-status <?php echo $pool['opening'] === null ? 'pending' : 'approved'; ?>"><?php echo $pool['opening'] === null ? '期初待核对' : '站内已记账'; ?></span></div>
    <div class="governance-pool-grid"><div><small>本任期目标额度</small><strong><?php echo $pool['opening'] === null ? '待核对' : '¥' . money($pool['opening']); ?></strong></div><div><small>已评审奖惩</small><strong><?php echo $pool['reviewed'] >= 0 ? '+' : '−'; ?>¥<?php echo money(abs($pool['reviewed'])); ?></strong></div><div><small>系统扣减</small><strong>−¥<?php echo money(abs($pool['penalties'])); ?></strong></div><div class="governance-pool-balance"><small>尚未定稿额度 · 非福利池余额</small><strong><?php echo $pool['balance'] === null ? '待核对期初' : '¥' . money($pool['balance']); ?></strong></div></div>
    <?php if ($pool['opening'] === null): ?><p class="governance-hint">该段历史尚未完整核对期初，不自动推算余额。</p><?php else: ?><p class="governance-hint">期初依据：<?php echo e($pool['source_note']); ?>。页面为本任期站内核算，最终福利结转须待任期结束并完成审核。</p><?php endif; ?>
    <?php if ($isCommittee && $quarterStart < '2026-09-15'): ?><details class="governance-review"><summary><?php echo $pool['opening'] === null ? '登记历史期初余额' : '更正历史期初余额'; ?></summary><form method="post"><input type="hidden" name="csrf" value="<?php echo e(ps_csrf_token()); ?>"><input type="hidden" name="action" value="set_pool_opening"><div class="form-row"><div class="form-group col-md-4"><label>核对后的期初余额</label><input class="form-control" type="number" name="opening_amount" min="0" max="1000000" step="0.01" value="<?php echo $pool['opening'] === null ? '' : e($pool['opening']); ?>" required></div><div class="form-group col-md-8"><label>来源 / 核对依据</label><input class="form-control" name="source_note" maxlength="500" value="<?php echo e($pool['source_note']); ?>" required placeholder="例如：8 月结余及 9 月已发放记录核对"></div></div><button class="btn btn-success btn-sm" type="submit">保存期初</button></form></details><?php endif; ?>
  </section>
  <section class="governance-card" id="rotation"><div class="governance-heading"><div><span class="governance-step">轮值日程</span><h2>谁在当值，从哪天开始</h2><p class="governance-hint">已确认栾鑫自 2026-09-15 起任至 12-14，历史补录区间不追扣。下一任达到两票一致后自动登记；如需人工调整特殊轮值期，请先核对不与现有任期重叠。</p></div></div>
