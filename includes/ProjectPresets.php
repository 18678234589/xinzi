...
function ps_preset_rules()
{
    ...
    return [
        ...
        // 网站售后部《网站售后部算法》
        $r('customer_service', '网站续费', '*', '续费', 'pool', 0, 0.03, 1, 0, '网站续费：录单每单 1 元；续费毛利（收入 − 成本 − 3%）按人比例在月度规则提成'),
        $r('customer_service', '网站续费', '*', '拍链接', 'pool', 0, 0.03, 0.5, 0, '拍链接订单：每单 0.5 元'),
        $r('customer_service', '网站修改', '*', '*', 'pool', 0.20, 0, 0, 0, '自己接单：(收入 − 成本) × 20%'),
        $r('customer_service', '网站修改', '*', '备案修改', 'pool', 0.20, 0, 5, 0, '备案修改：(收入 − 成本) × 20% + 每单 5 元'),
        ...
        // 商标部《提成算法》：
        // 商标客服：利润 = 售价 − 成本 − 售价×1%；提成 = 利润×12% + 单量补助（老客户 3 元/单、新客户 6 元/单）；小额返款每单 3 元。
        $r('customer_service', '商标', '*', '*', 'pool', 0.12, 0.01, 3, 0, '商标客服普通订单：(售价 − 成本 − 1%服务费)×12% + 每单补助 3 元', null, $neg),
        $r('customer_service', '商标', '*', '普通订单', 'pool', 0.12, 0.01, 3, 0, '商标客服普通订单：(售价 − 成本 − 1%服务费)×12% + 每单补助 3 元', null, $neg),
        $r('customer_service', '商标', '*', '新客户', 'pool', 0.12, 0.01, 6, 0, '商标客服新客户订单：(售价 − 成本 − 1%服务费)×12% + 每单补助 6 元', null, $neg),
        $r('customer_service', '商标', '*', '小额返款', 'pool', 0, 0, 3, 0, '商标客服小额返款订单：每单补助 3 元', null, $neg),
        // 资料专员 / 提交专员：按商标件数每件 2.2 元。
        $r('technical', '商标', '资料专员', '*', 'pool', 0, 0, 2.2, 0, '商标资料专员：每件 2.2 元'),
        $r('technical', '商标', '提交专员', '*', 'pool', 0, 0, 2.2, 0, '商标提交专员：每件 2.2 元'),
        $r('technical', '商标', '*', '*', 'pool', 0, 0, 2.2, 0, '商标技术专员：每件 2.2 元'),
    ];
}
