...
try {
    ...
    foreach ($people as $name => $id) if (!in_array($name, ['朱俊英', '李仁超', '孙磊', '乔立宾', '对账代写客服', '对账合接客服'], true)) $attendance->execute([$id, ['苏婷' => 24, '李子晖' => 1.84, '吴宁' => 9.52, '孙曼' => 2.96, '刘群' => 2.96, '魏慧子' => 12, '宋文娜' => 6.96][$name] ?? 0]);
    ...
    $manual = ['刘媛媛' => [2920.83, '网站续费提成'], '吴宁' => [750, '督导提成'], '曹双双' => [1125.80, '标书提成'], '王宁' => [777.85, '标书提成'], '王亚' => [5903.63, '续费等提成'], '乔立宾' => [1609.20, '代写主管提成（工资表未附算法）']];
